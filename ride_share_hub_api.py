...
async def rides(rider_details: Fetch_Rides_Request_Body):
    ...
    cur = conn.cursor()
    # Executing the SQL query
    rider_query = """Select rs.ride_id, c1.city as departure_city, c1.state as departure_state, c1.country as departure_country,
...
async def passenger_reservations(user_details: Fetch_Reservation_Request_Body):
    ...
    cur = conn.cursor()
    # Executing the SQL query
    bookings_search_query = """Select bks.booking_id, bks.number_of_seats_reserved, bks.booking_date, bks.booking_time, bks.billed_amount,
